...
<head>
    ...
    <style>
        ...
        body {
            ...
            /* Mencegah scrollbar horizontal yang mungkin muncul sesaat karena animasi */
            overflow-x: hidden;
        }
        .text-ceban-blue { color: var(--ceban-blue); }
        .border-primary { border-color: var(--ceban-blue) !important; }
        .btn-primary { background-color: var(--ceban-blue); border-color: var(--ceban-blue); }
        .btn-primary:hover { background-color: #2972c2; border-color: #2972c2; }
        .btn-outline-primary { color: var(--ceban-blue); border-color: var(--ceban-blue); }
        .btn-outline-primary:hover { background-color: var(--ceban-blue); color: white; }
...
<nav class="navbar navbar-expand-lg sticky-top fade-in-effect"> <div class="container">
        <a class="navbar-brand fw-bolder fs-4 text-ceban-blue" href="/">CeBan</a>
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav mx-auto gap-lg-2">
                ...
                <a class="nav-link active" aria-current="page" href="/status-banjir">Status Banjir</a>
                <a class="nav-link" href="/request-evakuasi">Request Evakuasi</a>
...
<div class="container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap fade-in-effect" style="animation-delay: 150ms;">
        <div>
    ...
    <div class="card shadow-sm mb-4 fade-in-effect" style="animation-delay: 300ms;">
        <div class="card-body">
            <form id="form-pencarian" class="row g-3 align-items-end">
                <div class="col-md-3">
                    <label for="tanggalInput" class="form-label"><i class="fas fa-calendar-alt me-1"></i> Tanggal</label>
                    <input type="date" class="form-control" id="tanggalInput" required>
                </div>
                <div class="col-md-3">
                    <label for="waktuInput" class="form-label"><i class="fas fa-clock me-1"></i> Waktu (hh:mm)</label>
                    <input type="time" class="form-control" id="waktuInput" required>
                </div>
                <div class="col-md-4">
                    <label for="searchStation" class="form-label"><i class="fas fa-water-ladder me-1"></i> Nama Pompa</label>
                    <input type="text" class="form-control" id="searchStation" placeholder="Cari nama pompa air...">
                </div>
                <div class="col-md-2">
...
<script>
    // Tidak ada perubahan pada JavaScript, semua berfungsi seperti sebelumnya.
    ...
    function displayResultsInGrid(results) {
        ...
        function getWeatherIcon(weather) {
            ...
            if (weatherLower.includes('cerah')) return { icon: 'fa-sun', colorClass: 'text-warning' };
            if (weatherLower.includes('berawan')) return { icon: 'fa-cloud', colorClass: 'text-secondary' };
            if (weatherLower.includes('hujan ringan')) return { icon: 'fa-cloud-rain', colorClass: 'text-info' };
            if (weatherLower.includes('hujan sedang')) return { icon: 'fa-cloud-showers-heavy', colorClass: 'text-primary' };
            if (weatherLower.includes('hujan lebat')) return { icon: 'fa-cloud-bolt', colorClass: 'text-ceban-blue' };
            return { icon: 'fa-smog', colorClass: 'text-muted' };
        }
        function getStatusInfo(status) {
            if (status === 'Aman') return { badge: 'bg-success', border: 'border-success', textColor: 'text-success' };
            if (status === 'Siaga') return { badge: 'bg-warning', border: 'border-warning', textColor: 'text-warning' };
            if (status === 'Waspada') return { badge: 'bg-danger', border: 'border-danger', textColor: 'text-danger' };
            return { badge: 'bg-secondary', border: 'border-secondary', textColor: 'text-dark' };
        }
